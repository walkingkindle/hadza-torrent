...
func DownloadFromPeer(downloadCtx context.Context, p peer.Peer, torrent types.TorrentFile, peerID [20]byte, state *download.DownloadStatus) error {
	...
	if err = download.Download(downloadCtx, &connection, torrent, state); err != nil {
		return err
	}
	return nil
}
